...
# ── STEP 1: extract way_tags from all 4 PBFs (CZ is the gap; others
# get re-extracted but ON CONFLICT DO UPDATE preserves correctness) ─
step 1 "extract way_tags from all 4 PBFs"
if ! $PG_RUN_PY extract_way_tags.py; then
  echo "!!! extract_way_tags FAILED — aborting"; exit 1
fi

# Sanity: CZ way_tags entries should now have highway populated.
# (Total way_tags row count doesn't change much because CZ osm_way_ids
# already existed with empty values from earlier extract — ON CONFLICT
# DO UPDATE updates them in place. The right check is highway coverage
# for a known CZ region.)
WT_CZ_HW=$($PSQL -qtA -c "
WITH cz_vid AS (
  SELECT id FROM ways_vertices_pgr
  WHERE the_geom && ST_MakeEnvelope(14.37, 50.03, 14.47, 50.13, 4326) LIMIT 500
), cz_ways AS (
  SELECT DISTINCT w.osm_way_id FROM cz_vid c JOIN ways w ON w.source = c.id
)
SELECT COUNT(*) FROM cz_ways cw JOIN way_tags wt USING (osm_way_id) WHERE wt.highway <> '';
" | tail -1)
echo "[recovery-b-waytags] CZ Prague 5km way_tags w/ highway: $WT_CZ_HW"
if [ "$WT_CZ_HW" -lt 50 ]; then
  echo "!!! way_tags missing CZ highway ($WT_CZ_HW) — CZ extract didn't merge correctly"
  exit 1
fi
