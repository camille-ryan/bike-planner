...
- Rail-verification scope: distinguish BASE OVERNIGHTS (where the user actually sleeps and where a partner meets them by train) from INTEREST DETOURS (side trips to nearby major cities where the user stays 2–3 days). Base overnights REQUIRE direct-train service to a hub. Interest detours can accept a one-transfer connection (the user rides there for a few days, no evening meet-up required). Only fail an interest detour if it's completely unreachable, not just non-direct.
