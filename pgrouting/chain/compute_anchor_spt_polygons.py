...
def _edges_by_endpoint(chain_edges: list[dict]
                       ) -> dict[str, list[tuple[str, list[list[float]], float]]]:
    """ref -> [(other_ref, geom_oriented_away_from_ref, cost_m), ...]
    Geom is oriented so geom[0] is at `ref` and geom[-1] is at `other_ref`."""
    out: dict[str, list[tuple[str, list[list[float]], float]]] = {}
    for e in chain_edges:
        a, b, geom = e["a"], e["b"], e["geom"]
        cost = float(e.get("cost_m", 0.0))
        out.setdefault(a, []).append((b, list(geom), cost))
        out.setdefault(b, []).append((a, list(reversed(geom)), cost))
    return out
...
def _build_polygon(anchor: dict,
    ...
    for b_ref, geom_ab, cost_ab in a_edges:
        if b_ref.startswith("ferry:"):
            ...
            # chain-Dijkstra, not the polygon SPT. Skip entirely: the
            # OTHER pier's polygon extends to Denmark on its own,
            # driven by ITS landward chain neighbors.
            if a_is_pier:
                continue
            # A is a land anchor with a pier neighbor — add just the
            # pier's location so A's SPT reaches the pier vertex on
            # the road network. No disc, no geometry walk, no 0.5-hop.
            if geom_ab:
